...
impl SyncEngineConfig {
    pub fn from_file(path: impl AsRef<Path>) -> Result<Self, config::ConfigError> {
        let content = std::fs::read_to_string(path.as_ref())
            .map_err(|e| config::ConfigError::Message(format!("Failed to read config file: {}", e)))?;
    ...
    pub fn from_env() -> Result<Self, config::ConfigError> {
        ...
        Ok(Self {
            node: NodeConfig {
                ...
                site_id: std::env::var("SYNC_SITE_ID")
                    .unwrap_or_else(|_| "site-default".into()),
            ...
            transport: TransportConfig {
                grpc_listen: std::env::var("SYNC_GRPC_LISTEN")
                    .unwrap_or_else(|_| "0.0.0.0".into()),
                grpc_port: std::env::var("SYNC_GRPC_PORT")
                    .ok().and_then(|v| v.parse().ok()).unwrap_or(50052),
                mesh_port: std::env::var("SYNC_MESH_PORT")
                    .ok().and_then(|v| v.parse().ok()).unwrap_or(50053),
            ...
            protocol: ProtocolConfig {
                ...
                compression_level_full: std::env::var("SYNC_COMPRESSION_FULL")
                    .ok().and_then(|v| v.parse().ok()).unwrap_or(19),
            ...
            security: SecurityConfig {
                tls_cert_path: std::env::var("SYNC_TLS_CERT")
                    .expect("SYNC_TLS_CERT must be set"),
                tls_key_path: std::env::var("SYNC_TLS_KEY")
                    .expect("SYNC_TLS_KEY must be set"),
                tls_ca_path: std::env::var("SYNC_TLS_CA")
                    .expect("SYNC_TLS_CA must be set"),
            ...
            observability: ObservabilityConfig {
                ...
                log_level: std::env::var("SYNC_LOG_LEVEL")
                    .unwrap_or_else(|_| "info".into()),
                log_format: std::env::var("SYNC_LOG_FORMAT")
                    .unwrap_or_else(|_| "json".into()),
                health_check_port: std::env::var("SYNC_HEALTH_PORT")
                    .ok().and_then(|v| v.parse().ok()).unwrap_or(8082),
            },
            ...
            events: EventConfig {
                ...
                max_event_size_bytes: std::env::var("SYNC_MAX_EVENT_SIZE")
                    .ok().and_then(|v| v.parse().ok()).unwrap_or(1024 * 1024),
                event_bus_type: std::env::var("SYNC_EVENT_BUS")
                    .unwrap_or_else(|_| "nats".into()),
    ...
    pub fn node_id_bytes(&self) -> [u8; 16] {
        let id = uuid::Uuid::parse_str(&self.node.node_id)
